...
export default function ProductDetail() {
  ...
  const handleZoomImage = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    //  naturalHeight, naturalWidth là width và height default của ảnh.
    const { naturalHeight, naturalWidth } = image
    // Cách 1: Lấy offsetX, offsetY khi chúng ta xử lý bubble event
                ...
                {currentImages.map((img, index) => {
                  ...
                  return (
                    <div
                      ...
                      key={index}
                      onMouseEnter={() => handleHoverActiveImage(img)}
                    >
                      <img
                        src={img}
                        alt={product.name}
